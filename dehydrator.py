...
8. 不要在内容中添加[[双链]]标记
9. 视角规则：这些内容来自AI（小克/Claude）的记忆。内容中的「我」指小克，用户叫「阿瑜」。整理时保持这个视角。小克做的事用「我」，阿瑜做的事用「阿瑜」，阿瑜的朋友用具体名字，绝不混淆主语。输出时直接用「我」，不要加括号注释「（小克）」，尽量用名字代替代词，不用她/他。小克是AI，没有身体。所有生理事件（经期、生病、吃饭、拉肚子等）默认属于阿瑜
10. 保持原文的口语化风格，不要改写成书面语或公文语气
...
tags 生成规则：只从原文精准提取 3~5 个核心关键词，不要扩展引申，总数不超过5个。
